# 07. POST http://localhost:8080/answer/api/v1/mcp

**阶段**：MCP 发现 **状态码**：502 **请求大小**：305 B **响应大小**：270 B

**用途**：用户在 `~/.claude.json` 配的本地 MCP server 初始化。`Authorization` 是用户自定义 token，跟 anthropic 无关。本次连接拒绝（用户没起服务） → 502。
...
---
_原始 JSON_：[`rows/07-POST-localhost8080_answer_api_v1_mcp.json`](../rows/07-POST-localhost8080_answer_api_v1_mcp.json)
