# 09. GET https://api.anthropic.com/api/oauth/account/settings

**阶段**：登录 — bootstrap **状态码**：200 **请求大小**：0 B **响应大小**：920 B

**用途**：拉 claude.ai 账户偏好（onboarding 状态、横幅 dismiss、是否启用 artifacts/citations/latex 等预览特性）。`claude-code/2.1.126` UA + `oauth-2025-04-20`。
...
| server-timing | x-originResponse;dur=107 |
...
---
_原始 JSON_：[`rows/09-GET-api.anthropic.com_api_oauth_account_settings.json`](../rows/09-GET-api.anthropic.com_api_oauth_account_settings.json)
