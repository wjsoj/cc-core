...
| `codexapp0.153.4/` | OpenAI ChatGPT backend / Codex **Desktop** | `Codex Desktop/0.153.4`, 2026-09-07 | **current Desktop target, and the only capture spanning a complete OAuth re-login.** Establishes that the three `/oauth/token` grants do **not** share a request shape (refresh is JSON and identifies itself; the auth-code and token-exchange legs are form-encoded and anonymous), that `expires_in` is 10 days with an `earliest_refresh_at` at 9, and that Desktop's WS handshake diverged from the CLI's — no `x-codex-turn-metadata`/`beta-features`/`routing-hint`, plus `x-openai-internal-codex-responses-lite` as an HTTP header. Also the first sighting of `wham/remote/control/server/refresh` and `x-codex-installation-id`. A codex-tui client was running concurrently, so the Desktop-vs-CLI diff carries no version drift. `/oauth/authorize` remains uncaptured. |
| `codexv0.153.4/` | OpenAI ChatGPT backend / Codex CLI | `codex-tui/0.153.4`, 2026-09-05 | **current CLI target.** Source of `gpt-6-astra` in the model catalog, the `remote_compaction_v2` beta-features convergence, the 15-field `x-codex-turn-metadata`, and the correction that `x-codex-routing-hint` IS sent on the WS upgrade. `codexws/capture_parity_test.go` reads its `rows/10` directly rather than a copy. |
| `codexapp0.147.0/` | OpenAI ChatGPT backend / Codex **Desktop** | `Codex Desktop/0.147.0-alpha.6.6`, 2026-08-14 + 08-15 | Superseded by `codexapp0.153.4/` for identity, but **kept**: it is the only record of the 0.147.0 Desktop handshake (which still sent `x-codex-turn-metadata`), and the only in-tree source of turn-opening `response.create` frames and their `input[]` items. |
| `codexv0.135.0/` | OpenAI ChatGPT backend / Codex CLI | `codex-tui/0.135.0`, 2026-05-30 | The first Codex capture; superseded by `codexv0.153.4/`. Retains the 5h quota window's retirement (2026-07, weekly-only) and the original CLI body shapes. See `codexv0.135.0/SPEC.md`. |
...
| `scripts/` | tooling | — | `extract_live.py` (Claude structural redactor), `extract_codex_live.py` (Codex; consumes whistle's `get-data?ids=` shape, `--profile=cli-session|desktop-login`), `sanitize.py`/`gen.py` (older pipeline) |
