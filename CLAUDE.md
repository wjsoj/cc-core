...
- `requestlog` — one JSON line per terminal request to a daily file, **plus a SQLite index (`requests.db`) inside the same directory** that `Query` / `AggregateByAuth` / `AggregateHourly` serve from. The JSONL stays the source of truth; the index is derived, rebuilt from it on demand, and the three functions fall back to scanning when no index is open, so **no call site changes and a broken index only costs speed**. A host opts in with `OpenStore(dir)` (registered per-directory, looked up by `Filter.Dir`). Ingest is an incremental tail read that self-heals on truncate, retention delete, and the equal-size in-place rewrite `RewriteClientMask` performs (mtime is the only signal there). Unbounded aggregates read daily rollups, not rows. Two traps: `req.day` is the **source file's UTC day** while `req.bday` is the **display-zone day** (production buckets in Asia/Shanghai, where they disagree) — changing the zone drops and rebuilds; and `attempt_only` rows are invisible to every query, matching `matches()`. Every index is partial on `attempt_only = 0` and carries the sort columns, because an index that doesn't cover `ORDER BY ts DESC, id DESC` silently degrades to a full sort.
