...
`rewrite` consumers use the prepared-request pipeline: `ClassifyClaudeCodeRequest(original)` → `NewClaudeCodeRequestPolicy` → `PrepareClaudeCodeRequest` → `ApplyClaudeCodePreparedRequest`. Preparation binds the body to the selected account key and credential kind; the final call validates that stable binding, reads the latest rotatable bearer, and installs both body and headers from the same opaque result. `rewrite` maps metadata to the selected OAuth account, maps the source session by account+client token, and pins the billing version plus UA/Stainless core profile. Custom-base-url Claude Code requests do not emit first-party `cch` or `cc_prev_req`; if either unexpectedly appears, preparation fails closed rather than forwarding a value the proxy cannot safely regenerate or rebind. Request `Anthropic-Beta` is retained because main, 1M, and title carry different feature vectors; a genuine rewrite with no downstream beta vector fails closed instead of inventing the wrong one. A preparation error must not trigger credential failover. The per-credential JSON field is `claude_identity_mode`; missing means `preserve`, legacy `rewrite_strip` is accepted as a read alias, new state is persisted as `rewrite`, and only Anthropic OAuth credentials accept it. **`preserve` is the A/B control contract: consuming applications must bypass the new prepared pipeline and run their complete pre-policy forwarding path** (slot source, switch detection, thinking sanitation/recovery, body shaping, and headers), rather than silently redefining the control as byte-exact passthrough. The lower-level `GenuineRequestPreserve` helper remains byte-exact for callers that explicitly need that primitive, but it is not a replacement for the application's legacy path. Runtime mode changes go through `Pool.UpdateClaudeIdentityMode`: the credential must already be disabled and have no active sticky sessions, so a virtual process cannot switch identity/profile mid-session.
