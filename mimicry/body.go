...
//     (bare — no cache_control, matches real 2.1.183).
...
func ApplyClaudeCodeBodyMimicry(body []byte, model string, id SimIdentity) []byte {
	...
	// Step 1: rebuild system to match the CC 2.1.183 layout.
	...
	// NOTE: real CC 2.1.183 also carries `thinking` ({"type":"adaptive"}),
...
// rewriteSystemForOAuth rebuilds the system field to match the real CC 2.1.183
...
func applySystemCacheBreakpoints(blocks []json.RawMessage) {
	...
	// Real CC 2.1.183 puts scope:global on the SECOND-TO-LAST system block
...
// last block of the last message — exactly what real CC 2.1.183 does
